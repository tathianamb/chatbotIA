...
def question_rewrite(query, vocab):
    terms = [word for word in query if word in vocab]
    return " ".join(terms)

def load_vocab(vocab_path):
    with open(vocab_path, 'r', encoding='utf-8') as file:
        vocab = [raw.strip().lower() for raw in file]
    return vocab

def test_questions_distance_strategies(vocabullary):
    ...
    for strategy, vector_store_path in vector_store_paths.items():
        ...
        for question in questions:
            chatbot.get_response(question, vector_store_path=vector_store_path, to_rewrite_query=True)
            time.sleep(5)
...
if __name__ == "__main__":
    ...
    vocab = load_vocab(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'src', 'controlled_vocab.txt'))

    test_questions_distance_strategies(vocabullary=vocab)
